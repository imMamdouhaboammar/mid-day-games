...
import { isActiveToday } from "../utils/playerProgress";
...
export const CasesList: React.FC<CasesListProps> = ({
  ...
  const recommendedCase = useMemo(
    () => cases.find((story) => !solvedCaseIds.has(story.id)) || cases[0],
    [cases, solvedCaseIds],
  );

  const filteredCases = cases.filter((story) => {
    const difficultyMatches =
      filterDifficulty === "all" || story.difficulty === filterDifficulty;
    const solved = solvedCaseIds.has(story.id);
    const statusMatches =
      filterStatus === "all" ||
      (filterStatus === "solved" && solved) ||
      (filterStatus === "unsolved" && !solved);

    return difficultyMatches && statusMatches;
  });

  const activeToday = isActiveToday(playerStats);
  const solvedCount = solvedCaseIds.size;
  const completionPercent =
    cases.length > 0 ? Math.round((solvedCount / cases.length) * 100) : 0;

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-7">
      <section className="border-b border-slate-800/80 pb-6 space-y-4" aria-labelledby="cases-heading">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <h2 id="cases-heading" className="text-xl sm:text-2xl font-bold font-novel text-slate-100 tracking-wide">
              استراحة ذهنية واعية
            </h2>
            <p className="text-xs sm:text-sm text-slate-400 mt-1 max-w-xl leading-relaxed">
              قضية قصيرة أو تمرين واحد يكفي لتسجيل جلسة اليوم. الهدف جلسة مركزة، لا وقت شاشة أطول.
            </p>
          </div>

          <div className="text-xs text-slate-400 sm:text-left">
            <div className="font-mono text-slate-200">{solvedCount}/{cases.length} قضية</div>
            <div>{completionPercent}% من المكتبة مكتمل</div>
          </div>
        </div>

        <div className="bg-[#0f1523] border border-slate-800/90 rounded-2xl p-4 sm:p-5 card-bezel">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2 text-xs font-semibold">
                ...
                <span className={activeToday ? "text-emerald-300" : "text-slate-200"}>
                  {activeToday ? "تم تسجيل نشاط اليوم" : "مهمة اليوم"}
                </span>
              </div>
              <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">
                {activeToday
                  ? `سلسلتك الحالية ${playerStats.streakDays} يوم. يمكنك التوقف هنا أو إكمال جولة إضافية.`
                  : "اختر قضية غير محلولة أو أكمل تمرين تنشيط قصير للحفاظ على التتابع."}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {recommendedCase && (
                <button
                  type="button"
                  onClick={() => onSelectCase(recommendedCase)}
                  ...
                  aria-label={`ابدأ قضية اليوم: ${recommendedCase.title}`}
                >
                  <BookOpen className="w-3.5 h-3.5" />
                  <span>قضية اليوم ({recommendedCase.estimatedMinutes} د)</span>
                </button>
              )}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-bold text-slate-200 font-novel">الملفات المتاحة</h3>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1 bg-slate-900/60 border border-slate-800/80 p-0.5 rounded-lg text-xs">
            ...
            {["all", "مبتدئ", "متوسط", "متقدم", "محقق عبقري"].map((diff) => (
              <button
                type="button"
                key={diff}
      ...
      {filteredCases.length === 0 ? (
        <div className="text-center py-12 bg-slate-900/40 border border-slate-800/80 rounded-2xl text-slate-400 text-sm">
          لا توجد قضايا تطابق الفلاتر الحالية.
        </div>
      ) : (
          ...
          {filteredCases.map((story) => {
            ...
            return (
              <button
                ...
                aria-label={`فتح قضية ${story.title}، المستوى ${story.difficulty}، المدة ${story.estimatedMinutes} دقائق`}
              ...
              >
                ...
                <div className="space-y-3 w-full">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-amber-400/80 font-medium mt-1">{story.chapterTitle}</div>
                  </div>
